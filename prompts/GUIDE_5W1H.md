...
대상 문서: `prompts/CORS_RINEX30S_GNSS_interference_V7_1.md` (V7.1)

이 설명서는 위 프롬프트를 **처음 보는 사람**이 "이게 무슨 일을 시키는 문서이고, 왜 이렇게 길고 까다로운가"를 이해하도록 쓴 안내서다. 원문은 3,500줄이 넘는 실행 계약이라 처음부터 읽기 어렵다. 여기서는 Why → What → Who → Where → When → How 순서로 풀어 설명하고, 마지막에 자주 하는 오해·용어·본문 위치 색인을 붙였다.
...
| **역할별 실행 런북** | **40절** |
| **병렬 검증·성능** | **41절** |
| **규칙이 없는 상황을 만났을 때** | **42절 (미규정 사례)** |
| **실행 세션이 끊길 때 / 세션 규칙** | **43절** |
| **이 자료 특유의 처리 규칙** | **44절 (세션 경계·canonical·nav·코호트·저장·후보율)** |
...
### V7.1이 더한 것 — 빈틈을 다루는 규칙

계약은 실제 데이터의 모든 사례를 미리 규정할 수 없다. 그래서 V7.1은 조항을 늘리는 대신 **빈틈 자체를 다루는 방법**을 넣었다.

- **42절 미규정 사례** — 규칙이 없는 상황에서 멈추지도 임의로 정하지도 말고, `UNSPECIFIED_CASES.csv`에 기록하고 보수적 기본값(제외 대신 격리 / 추정 대신 `unassessable` / 병합 대신 분리 / 승격 대신 보류)을 적용한 뒤 계속한다. 같은 사례가 쌓이면 그게 계약 개정 요청이다.
- **43절 실행 세션** — 에이전트의 작업 컨텍스트가 끊기면 판단이 사라지고 다음 세션이 다르게 해석한다. 세션 시작 시 필수 문서 읽기, 종료 시 판단 기록, 불변 조항 상시 주입, **한 세션 안에서 `blind_status`를 바꾸지 않기**를 강제한다.
- **44절 실측 반영** — 사전 감사에서 드러난 이 자료의 실제 특성(875,789파일 / 175,935 station-day / 454 GiB / 관측소 79→98국 / 전량 `.Z` / 형식 예외 10일)에서 따라오는 규칙: 자정 세션 경계 처리와 **파일 단위 중심화 금지**, canonical 파일 선택, nav 병합, 고정 코호트 비교, 위성×epoch 전량 저장 금지, 후보율·처리량 가드.
