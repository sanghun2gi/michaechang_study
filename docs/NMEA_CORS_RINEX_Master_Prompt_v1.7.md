# 이동체 NMEA + 고정 CORS RINEX 기반 박사논문 통합연구 Master Prompt v1.7
...
고정 CORS RINEX의 관측소·위성·주파수·constellation·네트워크 동시성을 이용하면 장비고장·자료결손·전리층·위성계통 이상과 구별되는 interference-compatible anomaly를 얼마나 재현가능하게 탐지할 수 있는가? 민감도·특이도·오경보율은 사건수준 검증라벨과 적절한 전체 분모가 확보된 경우에만 추정하고, 그렇지 않으면 후보율·false alarms per station-day·안정성·외부일치도로 범위를 제한한다.

### RQ4. 이동체–고정망 증거통합

`S_COMMON_SUPPORT`와 적절한 검증라벨이 확보된 경우, 두 관측계층의 late fusion은 각 데이터 단독 분석보다 사건 수준의 신뢰도·설명가능성·오경보 억제를 개선하는가? 라벨이 부족하면 성능향상 대신 concordance·독립 corroboration·불확실성 감소를 평가한다.
