# 이동체 NMEA + 고정 CORS RINEX 기반 박사논문 통합연구 Master Prompt v1.6
...
A_NMEA  = NMEA 단독연구가 blind하게 이상으로 판정한 날짜 집합
A_RINEX = RINEX 단독연구가 blind하게 이상으로 판정한 날짜 집합

A_UNION    = A_NMEA ∪ A_RINEX
A_SAME_DAY = A_NMEA ∩ A_RINEX
```

`A_NMEA`와 `A_RINEX`의 detector, feature, threshold, code/config hash, candidate list를 각각 먼저 동결한다. 한 연구의 이상일을 보고 다른 연구의 threshold나 feature를 조정하지 않는다. 공식 사건정보도 두 후보집합이 동결되기 전에는 열지 않는다.

#### 통합연구의 실제 조사대상

통합연구의 주 조사대상은 `A_UNION`이다. 즉, NMEA 또는 RINEX 중 어느 하나라도 이상이라고 판정한 모든 날짜에 대해 두 데이터의 내용을 동시에 교차조사한다.
...
한쪽 데이터가 없는 날은 다른 쪽이 정상이라고 간주하지 않고 `unassessable`로 둔다. 한쪽 데이터가 있으나 이상기준을 충족하지 않은 날도 상대 탐지기의 민감도·표본화·관측계층 차이를 검토한 후에만 불일치로 해석한다.

#### 대조일과 우연 일치 검정

`A_UNION`은 이상후보가 풍부한 선택표본이므로 이 자료만으로 전체기간의 이상 발생률, 민감도, 특이도 또는 오경보율을 추정하지 않는다. 같은 날 일치가 우연보다 많은지를 검정하기 위해 다음을 포함한다.
...
raw_score, calibrated_score, threshold_version
candidate_class
confounders_checked
counterevidence
evidence_level
...
다음 질문은 출발점이다. 실제 데이터 감사 후 `testable / partially_testable / untestable`을 판정하고 수정하라.

### RQ1. 관측가능성

저가형 이동체 수신기의 chipset·firmware 처리와 표준 NMEA 출력 제약은 GNSS 전파교란 양립 이상을 어디까지 관찰·구분할 수 있게 하며, 어떤 현상은 원천적으로 식별 불가능한가?

### RQ2. 이동체 NMEA 탐지

도로·속도·정지·터널·도심협곡·수목·multipath·수신기 차이 등의 혼란변수를 통제했을 때, NMEA-only 지표 조합으로 재현가능한 GNSS 서비스 이상후보를 얼마나 안정적으로 탐지할 수 있는가?

### RQ3. 고정 CORS 네트워크 탐지

고정 CORS RINEX의 관측소·위성·주파수·constellation·네트워크 동시성을 이용하면 장비고장·자료결손·전리층·위성계통 이상과 구별되는 interference-compatible anomaly를 어떤 민감도와 오경보율로 탐지할 수 있는가?

### RQ4. 이동체–고정망 증거통합

시공간 중첩이 존재할 경우, 두 관측계층의 late fusion은 각 데이터 단독 분석보다 사건 수준의 신뢰도·설명가능성·오경보 억제를 개선하는가?

### RQ5. 일반화와 주장수준

수신기·노선·지역·연도·관측소가 달라질 때 성능이 유지되는가? 어떤 증거조합부터 `anomaly`를 넘어 `interference-compatible`, `externally corroborated`, `confirmed`로 주장할 수 있는가?

각 RQ에 대해 다음 표를 작성한다.

```text
RQ | 필요한 자료 | 실제 보유여부 | 분석단위 | 독립변수 | 결과변수 |
혼란변수 | 검증방법 | 실패조건 | 허용 가능한 주장 | 필요한 추가자료
```

가설은 실제 라벨수와 시공간 중첩성이 확인된 뒤 설정한다. 라벨이 부족하면 감독학습 가설을 억지로 만들지 말고 탐색·검증 연구로 구분한다.
...
2. 독립 동결한 `A_NMEA`와 `A_RINEX`의 합집합을 immutable 통합조사 cohort로 구성
3. NMEA score와 RINEX score를 같은 확률이라고 가정하지 말고 각각 calibration
4. 라벨이 충분하면 투명한 rule baseline → 단순 calibrated model → 복잡모델 순으로 비교
5. 라벨이 부족하면 concordance, coincidence against null, permutation test, case-control event review 중심
6. NMEA-only / RINEX-only / fusion ablation
7. 외부자료가 있는 경우 event precision, recall, PR-AUC, false alarms/day, detection delay, Brier/ECE 등 적절한 지표 사용
8. 희귀사건에서 accuracy와 AUROC만으로 우수성을 주장하지 않음
9. 시간·공간 randomization 또는 matched negative windows로 우연한 중첩률 평가
10. 서로 다른 데이터에서 같은 결론이 나온 것과 같은 사건을 관측한 것을 구분
...
- E4-X/E5-C는 독립증거의 성격·품질·독립성·공간오차·시간오차를 명시한다.
...
두 폴더 내부의 논문·학위논문·보고서·표준·white paper·매뉴얼·PDF·HWP/HWPX·MD·DOCX·PPTX·BibTeX/RIS·참고문헌표·메모·기존 논문초안·코드 주석을 전수목록화한다. `reference`, `references`, `paper`, `literature`, `문헌`, `논문`, `원문`, `표준`, `manual`, `whitepaper`처럼 예상되는 폴더명만 보지 말고 전체 트리에서 관련자료를 찾는다.
...
### Phase 4 — 시공간 중첩·통합가능성 감사

- NMEA의 실제 운행일·시간·영역·수신기
- RINEX의 실제 관측기간·관측소·좌표·sampling
- `D_NMEA_VALID`, `D_RINEX_VALID`, `NMEA_LAST_VALID_DATE`, `RINEX_LAST_VALID_DATE`, `INTEGRATION_CUTOFF_DATE`
- 독립 동결된 `A_NMEA`, `A_RINEX`, `A_UNION`, `A_SAME_DAY`
- `A_UNION` 각 날짜의 NMEA 유효시간, RINEX coverage, 결측률과 품질게이트 통과율
- `BOTH_ANOMALY_SAME_DAY`, 단독 이상, 상대자료 없음, 시간·공간 불일치의 분포
- 최근접 관측소 거리·시간차·공간영향범위 및 `MATCHED_EVENT_CANDIDATE` 수
- 외부 사건자료의 가용기간·공간정밀도

을 계산하여 Mode A/B/C 중 권고안을 제시한다. 데이터 보유일의 교집합을 통합연구의 후보선정 기준으로 사용하지 않는다. 동일일 이상과 동일 사건을 혼동하지 않으며, 우연 일치 검정·대조일 수·통계적 검정력의 거친 범위를 함께 제시한다.
...
### Phase 6 이후 — Pilot → Scale → Blind Freeze → External Validation → Integration
...
6. 서로의 결과를 보지 않은 상태에서 `A_NMEA`와 `A_RINEX` 각각 blind freeze
7. `A_UNION`, `A_SAME_DAY`와 날짜별 cross-investigation matrix 생성
8. 시간·공간 매칭, 반증, 대조일·우연 일치 검정
9. 외부검증 및 전문가 독립검토
10. NMEA/RINEX/fusion ablation
11. 박사논문·논문별 표·그림·claim registry 확정
...
최초 실행은 Phase 0~2와 Phase 4의 **계획 가능한 범위**까지만 수행한다. 원시데이터 전수 scientific analysis나 모델학습은 시작하지 않는다.
...
4. **기존 연구자산과 진행상태**
5. **통합기간·후보일 판정** — 공통 종료일, `A_NMEA`, `A_RINEX`, `A_UNION`, `A_SAME_DAY`, 상대자료 가용성
...
- 이동체 궤적·개인정보의 윤리심의(IRB) 또는 CORS 자료의 기관 이용승인이 확인되지 않음
- 사건 좌표·시각의 공개 수준이 보안 민감시설 노출 위험을 배제하지 못함(§20.4)
...
- **개인정보/이동궤적**: NMEA 궤적은 차량·운전자 식별이 가능한 잠재적 개인정보로 취급한다. 소속기관 IRB(생명윤리위원회) 심의 대상 여부를 먼저 판정하고, 대상이면 심의 완료 전 신규 궤적 수집·확대 분석을 시작하지 않는다. 판정·근거·심의번호는 `26_data_ethics_review.md`에 기록한다.
- **CORS 자료 이용범위**: RINEX 자료의 출처기관(예: 국토지리정보원 등 CORS 운영기관)이 정한 이용약관·재배포·논문공개 범위를 확인하고, 106개 기준국 전국 자료 또는 그 파생 통계를 논문 부록·오픈데이터로 공개하기 전 별도 승인 필요 여부를 `27_data_use_agreement_status.md`에 `observed/configured/assumed/unavailable`로 기록한다.
- **이해상충(COI)**: 연구자와 자료 제공처(예: `E:\worldtechRnD\...` 경로가 시사하는 소속·거래관계)가 분리되지 않는 경우, 자료 소유권·IP·상업적 이해관계를 `28_conflict_of_interest_disclosure.md`에 명시하고 지도교수·심사위원에게 사전 고지한다.
- 위 세 산출물이 없거나 미완성 상태에서는 §15의 대규모 실행 단계(Phase 6 이후)로 진행하지 않는다.

### 20.2 통계적 엄밀성 보완 — 다중비교·검정력·사전등록

- **다중비교 보정**: 다수 관측소×다수 feature×다년 기간을 동시에 훑는 RINEX 네트워크 탐지와, 다수 이동체×다중 지표를 훑는 NMEA 탐지 모두에 FDR(Benjamini-Hochberg) 등 다중검정 보정 방법을 사전에 선택하고 동결한다. 공통 사건 객체(§5.1) 필드에 `multiple_testing_correction_method`, `correction_level`을 추가한다.
- **검정력/최소검출가능효과크기(MDES)**: 희귀사건 특성상 현재 표본·기간으로 어떤 크기의 효과까지 검출 가능한지 사전에 대략적 범위를 추정하고 `30_multiple_testing_and_power_plan.md`에 기록한다. 추정이 불가능하면 `unassessable`로 명시하고 이유를 남긴다.
- **사전등록**: §4.3/§5.0의 blind detection·code/config hash 동결과 별도로, 가능하면 기관 또는 OSF 등 제3자가 확인 가능한 사전등록(문서 해시·타임스탬프)을 `31_preregistration_record.md`로 남겨, 사후 threshold 조정 의혹을 원천적으로 배제한다. 여건상 불가능하면 그 사유를 기록하고 code/config hash 동결로 대체함을 명시한다.

### 20.3 국내 공식 외부검증 소스 목록화

§11.3의 외부 보완조사 대상은 국제표준·연구문헌 중심이다. 국내 사례 연구이므로 다음을 `32_domestic_external_corroboration_sources.md`에 기관·자료종류·접근절차·공개범위·신뢰순위와 함께 목록화하고, §9 증거등급의 E5-C 판정 시 우선 검토 소스로 사용한다. 실제 접근 가능 여부와 절차를 확인하기 전에는 `configured`로만 기록한다.

- 국립전파연구원(전파혼신 신고·전파환경측정망)
- 해양수산부/해양경찰청(연안 GPS 전파교란 신고)
- 국토교통부/항공안전기술원(NOTAM 등 항공 통보자료)
- 관계기관 공식 발표(안보 관련 교란 사건)
- 한국천문연구원 우주환경센터(우주기상·전리층 반증자료)
- GPSJam 등 민간 크라우드소싱 지도(§11.3 인용원칙에 따라 독립성·재인용 여부 검증 후 보조자료로만 사용)
...
2021~2026의 장기간 동안 위성군·신호·수신기/CORS 장비 변경은 §4.1/R0-R에서 이미 다루지만, **교란 기법 자체의 진화**(특히 SDR 기반 정교한 spoofing 등장)는 별도로 다루지 않았다. `34_temporal_drift_and_generalization_note.md`에 다음을 기록하고, 결론(§12 8장)의 한계 서술에 반드시 반영한다.
...
- RINEX/NMEA 파싱 라이브러리 선택과 근거(예: georinex, RTKLIB, teqc, pynmea2 등)
- 대용량 원자료·중간산출물 버전관리 방식(예: DVC, git-lfs)
...
`37_milestone_timeline.md`에 §13 Phase 0~6, 두 논문 투고 시점, 박사논문 심사 일정(제안발표·중간심사·최종심사)을 하나의 시간축으로 정리하고, `14_professor_meeting_brief.md`의 의사결정 요청사항과 연동한다.
